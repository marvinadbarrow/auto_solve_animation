...
if(remainingCardsArr.length > 0 && pickCardTracker.length > 0){
...
if(destinationPileName.includes('foundation')){
  ...
  // this undo happens just after the waste pile, full of remain cards, has all cards transferred to the pick pile which was previously empty; leaving the pick pile tracker with all remain card objects, and the waste pile tracker empty. 

 
  // the bread crumb has a special card value; the string 'all pick pile' to indicate that the move involved moving all waste cards back to pick pile. It is the only breadcrumb object that does not have an integer value for the card value. 

   let cardValue = lastBreadcrumb.primary_card.card
      // as a precaution double check the card value is a string
if(typeof cardValue == 'string'){

  console.log('all waste pile contents breadcrumb')
  console.log(lastBreadcrumb)

  // the pick length should be the total number of remain card objects available
  let pickLength = pickCardTracker.length
 
  // give all elements of the pick card tracker a when_moved value which is the same as the card object's when_flipped value - this will mean that the objects will have unique when_moved values, based on when they originally left the pick pile tracker
  pickCardTracker.forEach(object =>{
 
    // create a new object for sending back to the waste pile for each pick card object
    let newWasteObj = {
      ... object
    }
    // modify when moved property should revert when flipped value
    newWasteObj.when_moved = newWasteObj.when_flipped
    // card on the pick bile have the following origin/destination properties
    newWasteObj.primary_card.destination = 'waste-pile'
    newWasteObj.primary_card.origin = 'pick-pile'
    // put the object back into the wasteCardTracker array, unshifting so that the last object in pick tracker goes to the zero position of waste tracker as per the normal layout
    wasteCardTracker.unshift(newWasteObj)

    if(wasteCardTracker.length === pickLength){ // all objects have been transferred (this is before the pick card tracker has been emptied)
      // clear pick card tracker
      pickCardTracker = []
console.log('waste pile length')
console.log(wasteCardTracker.length)
    }
  })
// pop the object with special card value out of the breadcrumb array
  breadcrumbArray.pop()
     }else{
       alert('breadcrumb object has no card value')
     }
